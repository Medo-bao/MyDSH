...
const TOP_LEVEL_LABEL_KEYS = Object.freeze({
  file: "menu.file",
  edit: "menu.edit",
  view: "menu.view",
  window: "menu.window",
  help: "menu.help",
} satisfies Record<Exclude<MenuKind, "app">, DesktopMessageKey>);

const ROLE_LABEL_KEYS: Readonly<Record<string, DesktopMessageKey>> = Object.freeze({
  about: "menu.about",
  services: "menu.services",
  hide: "menu.hide",
  hideothers: "menu.hideOthers",
  unhide: "menu.unhide",
  quit: "menu.quit",
  undo: "menu.undo",
  redo: "menu.redo",
  ...
  copy: "menu.copy",
  paste: "menu.paste",
  pasteandmatchstyle: "menu.pasteAndMatchStyle",
  delete: "menu.delete",
...
export function bindShortcutMenu(
  ...
  const rebuild = (): void => {
    ...
    localizeBaseMenu(entries, locale);
    injectActions(
      entries,
      locale,
      bindings,
...
function localizeBaseMenu(
  ...
  for (const entry of entries) {
    if (entry.kind !== undefined && entry.kind !== "app") {
      entry.template.label = locale.t(TOP_LEVEL_LABEL_KEYS[entry.kind]);
    }
    localizeRoleItems(entry.template, locale);
  }
}

function localizeRoleItems(
  item: MenuItemConstructorOptions,
  locale: ShortcutMenuLocale,
): void {
  if (typeof item.role === "string") {
    const key = ROLE_LABEL_KEYS[item.role.toLowerCase()];
    if (key !== undefined) item.label = locale.t(key);
  }
  if (!Array.isArray(item.submenu)) return;
  for (const child of item.submenu) {
    localizeRoleItems(child, locale);
  }
}
...
function classifyTopLevel(
  ...
  if (
    item.role === "editMenu" ||
    submenuContainsRole(item, "undo") ||
    submenuContainsRole(item, "selectAll")
  ) {
    return "edit";
  }
  if (
    item.role === "windowMenu" ||
    submenuContainsRole(item, "minimize")
  ) {
    return "window";
  }
  if (item.role === "help") return "help";
  return undefined;
}
