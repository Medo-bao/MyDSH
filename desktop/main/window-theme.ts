...
type WindowThemeHost = Readonly<{
  setTitleBarOverlay?: (options: WindowTitleBarOverlay) => void;
  webContents: {
    ipc: {
...
export type WindowTitleBarOverlay = Readonly<{
  color: string;
  height: number;
  symbolColor: string;
}>;

/** Keep native caption controls legible on the dedicated title-bar surface. */
export function windowTitleBarOverlay(
  colorScheme: WindowColorScheme,
): WindowTitleBarOverlay {
  return colorScheme === "dark"
    ? { color: "#00000000", height: 44, symbolColor: "#d9dde3" }
    : { color: "#00000000", height: 44, symbolColor: "#454b55" };
}
