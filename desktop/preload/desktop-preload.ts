...
import {
  ...
  type WindowMenuPopupRequest,
} from "@minke/desktop/window-menu-contract.ts";
...
const windowMenu = Object.freeze({
  async popup(request: WindowMenuPopupRequest): Promise<void> {
    await ipcRenderer.invoke(
      WINDOW_MENU_POPUP_CHANNEL,
      parseWindowMenuPopupRequest(request),
    );
  },
});
...
contextBridge.exposeInMainWorld(
  ...
  Object.freeze({
    market: Object.freeze({
      ...
      restart: () => ipcRenderer.invoke(MARKET_RESTART_CHANNEL),
    } satisfies MarketPort),
    about,
    files,
    locale,
    modelRuntime,
