...
interface DesktopWindowMenuBridge {
  popup(request: {
    kind: "file" | "edit" | "view" | "help";
    x: number;
    y: number;
  }): Promise<void>;
}
...
interface DesktopBridgeWindow {
  minkeDesktop?: {
    ...
    windowMenu?: DesktopWindowMenuBridge;
...
export function hasWindowsDesktopSurface(
  source: DesktopBridgeWindow =
    window as unknown as DesktopBridgeWindow,
): boolean {
  return source.minkeDesktop?.surface?.kind === "windows";
}

export function desktopWindowMenuPort(
  source: DesktopBridgeWindow =
    window as unknown as DesktopBridgeWindow,
) {
  const bridge = source.minkeDesktop?.windowMenu;
