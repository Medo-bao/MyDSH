...
import { MarketManagement, installMarketManagementStyles, marketZh, marketEn, type MarketTranslate, type MarketManagementProps } from "./market-management/index.tsx";
import {
...
interface SlotService {
  register(options: {
    name: "settings.section"; id: "firefly-market-management"; order: number;
    label: () => string; locale: string; inject: () => MarketManagementProps;
  }, component: ComponentType<MarketManagementProps>): unknown;
  inject(name: string, callback: () => unknown): void;
  register(
    options: {
      name: "sidebar.brand.mark" | "conversation.hero.brand.mark";
      priority: -100;
    },
...
export function apply(ctx: HarnessClientContext): void {
  const marketNamespace = "firefly.market-management";
  ctx.effect(() => ctx.locale.register(marketNamespace, { zh: marketZh, en: marketEn }), "desktop: market management locale");
  ctx.effect(() => installMarketManagementStyles(), "desktop: market management styles");
  const marketT = ctx.locale.bind<keyof typeof marketZh>(marketNamespace) as MarketTranslate;
  ctx.slots.inject("settings.section", () => ctx.slots.register({
    name: "settings.section", id: "firefly-market-management", order: 90,
    label: () => marketT("title"), locale: marketNamespace, inject: () => ({ t: marketT }),
  }, MarketManagement));
  ctx.effect(
  ...
  if (hasWindowsDesktopSurface()) {
    ctx.slots.inject("sidebar.brand.mark", () =>
      ctx.slots.register(
        { name: "sidebar.brand.mark", priority: -100 },
        FireflyBrandMark as ComponentType<FireflyBrandMarkProps>,
      ),
    );
  ...
  if (hasWindowsDesktopSurface()) {
    const windowMenu = desktopWindowMenuPort();
    ctx.effect(
      () => installDesktopSurface(document, {
        ...
        labels: {
          toggleSidebar: t("action.toggleSidebar"),
          back: t("action.sessionBack"),
          forward: t("action.sessionForward"),
          file: t("toolbar.file"),
          edit: t("toolbar.edit"),
          view: t("toolbar.view"),
          help: t("toolbar.help"),
      ...
      "minke-overlay: native desktop toolbar",
    );
  }
  ctx.effect(
