...
This package is MyDSH's product-owned extension layer for DeepSeek Harness.
...
- `model-runtime` is a DSH plugin that owns local model discovery and optional
  ...
  context window. An externally started service is never reconfigured: MyDSH
  reports the current and required context lengths and asks the user to reload
  the model in LM Studio. If MyDSH started the service itself, it may load or
...
- `ensure-running` starts a missing service. LM Studio's one-shot CLI leaves
  the shared service running; an `ollama serve` process started by MyDSH is
...
MyDSH. `LM_STUDIO_BASE_URL` and `OLLAMA_BASE_URL` can select explicit loopback
endpoints and are also applied to services MyDSH starts. Without an override,
MyDSH follows the runtimes' official defaults: LM Studio uses
...
The browser half owns MyDSH's product policy, configurable keyboard shortcuts,
...
- Harness's `ctx.theme` snapshot and `theme/change` event for native window
  synchronization;
- a lifecycle-managed DOM adapter for the desktop sidebar, titlebar, and
...
- the isolated MyDSH preload bridge for durable desktop-owned preferences.
