...
const desktopPlatforms = Object.freeze(["win32"]);

function capture(command, args, cwd) {
  const safeArgs =
    command === "git"
      ? ["-c", `safe.directory=${resolve(cwd)}`, ...args]
      : args;
...
async function verifyProductBundle(projectRoot, harnessRoot, contract) {
  ...
  if (bundle.packageName !== "@firefly-harness/bundle-companion") {
    throw new Error(
      "Firefly product package must be @firefly-harness/bundle-companion.",
    );
  }
...
export async function verifyHarnessContract(projectRoot) {
  ...
  if (
    contract.runtimeSizeBudgetBytes === null ||
    typeof contract.runtimeSizeBudgetBytes !== "object" ||
    Array.isArray(contract.runtimeSizeBudgetBytes) ||
    Object.keys(contract.runtimeSizeBudgetBytes).length !== 1 ||
    !("win32" in contract.runtimeSizeBudgetBytes)
  ) {
    throw new Error(
      "Harness contract runtimeSizeBudgetBytes must only declare win32.",
    );
  }
