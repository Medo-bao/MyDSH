...
const runtimeMetadataVersion = 4;
const runtimeFingerprintPaths = [
  "config/companion-plugins.json",
...
function parseFlags(argv) {
  ...
  return {
    ...
    reuseBuild: argv.includes("--reuse-build"),
  };
}
...
async function runPnpm(args, cwd) {
  ...
  const effectiveArgs =
    registry === undefined || registry === ""
      ? args
      : [...args, `--config.registry=${registry}`];
  const invocation = resolvePnpmInvocation(effectiveArgs);
...
async function readCompanionPlugins() {
  const config = JSON.parse(
    await readFile(join(projectRoot, "config", "companion-plugins.json"), "utf8"),
  );
  if (!Array.isArray(config.plugins)) {
    throw new Error("companion plugin registry must contain a plugins array");
  }
  const plugins = [];
  const ids = new Set();
  const names = new Set();
  ...
  for (const entry of config.plugins ?? []) {
    if (
      ...
      throw new Error("companion plugin registry contains an invalid id, name, or directory");
    }
    if (ids.has(entry.id) || names.has(entry.name) || directories.has(entry.directory)) {
      throw new Error(`companion plugin registry contains a duplicate entry for ${entry.name}`);
    }
    ids.add(entry.id);
    ...
    if (manifest.name !== entry.name || typeof manifest.version !== "string") {
      throw new Error(`invalid companion package ${entry.directory}`);
    }
    plugins.push({ ...entry, packageRoot, version: manifest.version });
  }
  return plugins;
}

async function injectCompanionPackages(runtimeRoot, plugins) {
  for (const plugin of plugins) {
    ...
    const parent = dirname(destination);
    await mkdir(parent, { recursive: true });
    const candidate = await mkdtemp(join(parent, `.${plugin.directory}-staging-`));
    try {
      await cp(plugin.packageRoot, candidate, {
        dereference: true,
        preserveTimestamps: true,
        recursive: true,
      });
      await pruneRuntimeArtifacts(candidate);
      await publishDirectory(candidate, destination);
    } finally {
      await rm(candidate, { recursive: true, force: true });
    }
...
async function exposeProductBundleToProfiles(
  ...
  manifest.dependencies = {
    ...
    ...Object.fromEntries(
      companionPlugins.map((plugin) => [plugin.name, plugin.version]),
    ),
  };
  await writeFileAtomic(
    cliManifestPath,
...
async function writeRuntimeAdapters(
  ...
  contract,
  commit,
  { companionPackages, coreFingerprint, productBundleFingerprint, runtimeSize },
) {
...
exec "$SCRIPT_DIR/../node/node" "$@"
`,
    pnpm: `#!/bin/sh
set -eu
: "\${DSH_PNPM_ENTRY:?DSH_PNPM_ENTRY is required}"
SCRIPT_DIR="$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)"
exec "$SCRIPT_DIR/../node/node" "$DSH_PNPM_ENTRY" "$@"
`,
    pnpx: `#!/bin/sh
set -eu
: "\${DSH_PNPM_ENTRY:?DSH_PNPM_ENTRY is required}"
SCRIPT_DIR="$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)"
exec "$SCRIPT_DIR/../node/node" "$DSH_PNPM_ENTRY" dlx "$@"
`,
  };
  ...
  await writeFileAtomic(
    ...
    '@echo off\r\n"%~dp0..\\node\\node.exe" "%DSH_PNPM_ENTRY%" %*\r\n',
  );
  await writeFileAtomic(
    join(binRoot, "pnpx.cmd"),
    '@echo off\r\n"%~dp0..\\node\\node.exe" "%DSH_PNPM_ENTRY%" dlx %*\r\n',
  );
  await writeFileAtomic(
...
async function validateRuntime(
  runtimeRoot,
  contract,
  { companionPackages, commit, coreFingerprint, productBundleFingerprint, runtimeSize },
) {
  ...
  const required = [
    ...
    ...companionPackages.map((plugin) =>
      join(runtimeRoot, "node_modules", ...plugin.name.split("/"), "package.json"),
    ),
  ...
  if (
    ...
    JSON.stringify(metadata.companionPackages ?? []) !==
      JSON.stringify(companionPackages) ||
...
async function main() {
  if (process.platform !== "win32" || process.arch !== "x64") {
    throw new Error(
      `Harness staging requires win32-x64, received ${process.platform}-${process.arch}`,
    );
  }
  ...
  const companionPlugins = await readCompanionPlugins();
  const companionPackages = companionPlugins.map(({ id, name, version }) => ({
    id,
    name,
    version,
  }));
  ...
  if (flags.skipInstall && flags.skipBuild) {
    ...
    await injectCompanionPackages(activeRuntimeRoot, companionPlugins);
  ...
  if (flags.skipBuild || flags.reuseBuild) {
    console.log(
      `Skipping Harness source build (${flags.reuseBuild ? "--reuse-build" : "--skip-build"})`,
    );
  } else {
  ...
  try {
    ...
    await injectCompanionPackages(candidateRuntimeRoot, companionPlugins);
    await exposeProductBundleToProfiles(
      candidateRuntimeRoot,
      contract,
      productBundle,
    ...
    await cp(process.execPath, bundledNode, { preserveTimestamps: true });
